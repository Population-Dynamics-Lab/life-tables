...
# ---- cache + read (Ontario survivorship) ----
dir.create("data-cache", showWarnings = FALSE)
u_lt <- "http://www.prdh.umontreal.ca/BDLC/data/ont/fltper_5x5.txt"
f_lt <- file.path("data-cache", basename(u_lt))
suppressWarnings(try(
  utils::download.file(u_lt, f_lt, mode = "wb", quiet = TRUE),
  silent = TRUE
))
lt <- tryCatch(
  readr::read_table(u_lt, skip = 2),
  error = function(e) readr::read_table(f_lt, skip = 2)
)
lt <- lt |>
  filter(Year == "2015-2019") |>
  mutate(x = c(0, 1, seq(5, 110, by = 5)), n = lead(x, default = Inf) - x)
...
# ---- cache + read (Deaths) ----
u_D <- "https://www.prdh.umontreal.ca/BDLC/data/ont/Deaths_5x1.txt"
f_D <- file.path("data-cache", basename(u_D))
suppressWarnings(try(
  utils::download.file(u_D, f_D, mode = "wb", quiet = TRUE),
  silent = TRUE
))
Deaths <- tryCatch(
  readr::read_table(u_D, skip = 2, col_types = "ccddd"),
  error = function(e) readr::read_table(f_D, skip = 2, col_types = "ccddd")
)
# ---- cache + read (Population) ----
u_P <- "https://www.prdh.umontreal.ca/BDLC/data/ont/Population5.txt"
f_P <- file.path("data-cache", basename(u_P))
suppressWarnings(try(
  utils::download.file(u_P, f_P, mode = "wb", quiet = TRUE),
  silent = TRUE
))
Pop <- tryCatch(
  readr::read_table(u_P, skip = 2, col_types = "ccddd"),
  error = function(e) readr::read_table(f_P, skip = 2, col_types = "ccddd")
)
...
# ---- cache + read (Deaths Lexis, for cohort section) ----
u_lex <- "https://www.prdh.umontreal.ca/BDLC/data/ont/Deaths_lexis.txt"
f_lex <- file.path("data-cache", basename(u_lex))
suppressWarnings(try(
  utils::download.file(u_lex, f_lex, mode = "wb", quiet = TRUE),
  silent = TRUE
))
raw_lexis <- tryCatch(
  readr::read_table(u_lex, skip = 1, col_types = readr::cols()),
  error = function(e) {
    readr::read_table(f_lex, skip = 1, col_types = readr::cols())
  }
)
clt <- raw_lexis |>
  ...
  mutate(
    age = cut(
      ...
      labels = c(
        "0",
        "1-4",
        str_c(seq(5, 100, by = 5), "-", seq(9, 104, by = 5))
      )
    )
  ) |>
  # group by ages
